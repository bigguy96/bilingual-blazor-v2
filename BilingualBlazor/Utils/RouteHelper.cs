...
namespace BilingualBlazor.Utils
{
    public static class RouteHelper
    {
        public static string GetAlternatePath(string path, string twoLetterRequestCulture)
        {
            ...
            //var t = routables.Select(s => new
            //        {RouteAttributes = s.GetCustomAttributes(inherit: true).OfType<RouteAttribute>()})
            //    .SingleOrDefault(x => x.RouteAttributes.Any(a => a.Template.Contains(current)));
